...
const props = defineProps({
  kyt: { type: Object, default: null },
  penanganan: { type: Object, default: null },
})
...
onMounted(() => {
  ...
  // Load existing foto_path if available
  if (props.penanganan?.foto_path) {
    FabricImage.fromURL(assetUrl(props.penanganan.foto_path)).then((img) => {
      const cw = canvas.getWidth(); const ch = canvas.getHeight()
      const sc = Math.min(cw / (img.width || 1), ch / (img.height || 1))
      img.set({ originX: 'left', originY: 'top', scaleX: sc, scaleY: sc, left: (cw - (img.width || 0) * sc) / 2, top: (ch - (img.height || 0) * sc) / 2, selectable: true, evented: true })
      img.setCoords(); canvas.add(img); canvas.renderAll(); saveState()
    })
  }
...
async function submitForm(e) {
  ...
  if (!ok) return
  form.submit(route(penangananupdate({ id: props.penanganan.id })), {
    preserveScroll: true,
    onSuccess: () => toast.success('Penanganan berhasil diperbarui!'),
        ...
        <CardContent>
          <div class="grid gap-4">
            <div class="grid gap-2">
              <Label for="title">Masukkan Title:
            ...
            <div class="grid gap-2">
              <Label for="foto_path">Upload Gambar Penanganan (kosongkan jika tidak ingin mengganti):</Label>
        ...
        <CardContent class="p-4">
          ...
          <div class="mt-4">
            <Button @click="submitForm" class="w-full" :disabled="form.processing || !form.title">
              ...
              <template v-else-if="form.title">Simpan Perubahan Penanganan</template>
